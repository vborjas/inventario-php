...
<?php
  ...
  if(isset($_POST['add_herramienta'])){
    ...
    validate_fields($req_fields);
        if(empty($errors)){
		 ...
		 /* $s_total   = $db->escape($_POST['total']);*/
          ...
          $sql .= " product_id,DNI,usuario,negocio,detalle,qty,estado_usu,estado_entrega,date";
		  
          $sql .= ") VALUES (";
          $sql .= "'{$p_id}','{$s_DNI}','{$s_usuario}','{$s_negocio}','{$s_detalle}','{$s_qty}','{$s_estado_usu}','{$s_estado_entrega}','{$s_date}'";
          $sql .= ")";

                if($db->query($sql)){
                  update_product_qty($s_qty,$p_id);
                  $session->msg('s',"Activo Agregado ");
                  redirect('add_herramienta.php', false);
                } else {
    ...
    <div class="panel panel-default">
      <div class="panel-heading clearfix">
        <strong>
          ...
          <span>Registro Entrega de Equipos</span>
       </strong>
      </div>
      <div class="panel-body">
        <form method="post" action="add_herramienta.php">
